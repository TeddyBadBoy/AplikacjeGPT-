...
export const config={api:{bodyParser:{sizeLimit:'5mb'}},maxDuration:60};
const SITE_ORIGIN='https://osint-navigator.wojciechwoytynowski.chatgpt.site';
const MAX_INLINE=3.5*1024*1024;

function cors(req,res){
  const origin=String(req.headers.origin||'');
  if(origin===SITE_ORIGIN||origin==='https://chatgpt.com') res.setHeader('Access-Control-Allow-Origin',origin);
...
function parseDataUrl(input){
  ...
  if(!buffer.length||buffer.length>MAX_INLINE) throw new Error('Inline image must be between 1 byte and 3.5 MB.');
...
export default async function handler(req,res){
  ...
  try{
    ...
    return send(req,res,200,{
      ...
      status:a.candidates.length?'CANDIDATE_FOUND':'NO_COORDINATES_FOUND',
